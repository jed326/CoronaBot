...
function isPACounty(loc) {
  let counties = ["adams", "allegheny", "armstrong", "beaver", "berks", "blair", "bradford", "bucks", "butler", "cambria", "cameron", "carbon", "centre", "chester", "clarion", "clearfield", "columbia", "crawford", "cumberland", "dauphin", "delaware", "erie", "fayette", "franklin", "greene", "huntingdon", "indiana", "juniata", "lackawanna", "lancaster", "lawrence", "lebanon", "lehigh", "luzerne", "lycoming", "mckean", "mercer", "mifflin", "monroe", "montgomery", "montour", "northampton", "northumberland", "perry", "philadelphia", "pike", "potter", "schuylkill", "snyder", "somerset", "susquehanna", "tioga", "union", "venango", "warren", "washington", "wayne", "westmoreland", "york"];
...
function getPACountyCases(user, userID, channelID, message, evt, loc) {
  let route = "https://www.health.pa.gov/topics/disease/coronavirus/Pages/Cases.aspx";
  ...
  if (isPACounty(loc)) {
    request(route, function(err, response, body) {
      if (err) {
        ...
        let time = dom.window.document.getElementsByClassName("ms-rteStyle-Quote")[0].innerText.substring(dom.window.document.getElementsByClassName("ms-rteStyle-Quote")[0].innerText.search(' at ') + 4);
        let list = dom.window.document.getElementsByTagName("tbody")[3].children;
        let positve, dead;
        for(var i = 1; i < list.length; i++) {
          if (loc == dom.window.document.getElementsByTagName("tbody")[3].children[i].children[0].innerText.toLowerCase()) {
            positive = dom.window.document.getElementsByTagName("tbody")[3].children[i].children[1].innerText;
            dead = dom.window.document.getElementsByTagName("tbody")[3].children[i].children[2].innerText;
            dead = (dead=='\n')? '0': dead;
            bot.sendMessage({
              to: channelID,
              message: `...\n` +
...
bot.on("message", function(user, userID, channelID, message, evt) {
  ...
  if (message.substring(0, 1) == "!") {
    ...
    } else if (cmd.startsWith("PA ") || cmd.startsWith("pa ")) {
      getPACountyCases(user, userID, channelID, message, evt, cmd.split(' ')[1]);
